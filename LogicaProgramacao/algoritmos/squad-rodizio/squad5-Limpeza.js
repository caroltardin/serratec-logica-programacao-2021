...
do{
    console.log("CHECK A LIMPEZA DA CASA")

    console.log(`
    1- Sala de Estar
    2- Banheiro do 1º Andar
    3- Banheiro do 2º Andar
    4- Corredor 1º Andar
    5- Corredor 2º Andar
    6- Cozinha 
    7- Suíte
    ...
    local = parseInt(prompt("Nº da sua opção: "))
    limpoSujo = parseInt(Math.random() * 2)

   
    if (limpoSujo == 1)
        var statusLimpeza = "limpo"
    else
        statusLimpeza = "sujo"

    console.log(`O comodo ${casa[local -1]} está ${statusLimpeza}`)
    
    var checarOutro = prompt("Deseja checar outro cômodo?")
    var checarOutroTrue = checarOutro == "S" || checarOutro == "s"
    
    casa.shift(local)


}while(checarOutroTrue)
*/    
